...
contract MembershipVerificationToken is IERC1261, Ownable, ERC165 {
    ...
    mapping(uint256 => bytes32[]) public attributeValueCollection;
    ...
    event ApprovedMembership(address indexed _to, uint256[] attributeIndexes);
    event RequestedMembership(address indexed _to);
    event Assigned(address indexed _to, uint256[] attributeIndexes);
    event Revoked(address indexed _to);
    event Forfeited(address indexed _to);
    event ModifiedAttributes(
        address indexed _to,
        uint256 attributeIndex,
        uint256 prevValueIndex,
        bytes32 prevValue,
        uint256 modifiedValueIndex,
    ...
    function requestMembership(uint256[] _attributeIndexes) external payable {
    ...
    function assignTo(address _to, uint256[] _attributeIndexes) external onlyOwner {
    ...
    function modifyAttributeByIndex(address _to, uint256 _attributeIndex, uint256 _modifiedValueIndex) external onlyOwner {
        ...
        uint256 prevIndex = currentHolders[_to].data[_attributeIndex];
    ...
    function getCurrentMemberCount() external view returns (uint256) {
    ...
    function getAttributes(address _to) external view returns (uint256[]) {
        require(_to != address(0), "Address cannot be zero");
        return currentHolders[_to].data;
    }

    function getAttributeExhaustiveCollection(uint256 _index) external view returns (bytes32[]) {
        return attributeValueCollection[_index];
    }

    function getAttributeByIndex(address _to, uint256 _attributeIndex) external view returns (uint256) {
        require(currentHolders[_to].data.length > _attributeIndex, "data doesn't exist for the user");
    ...
    function _assign(address _to, uint256[] _attributeIndexes) internal {
        require(_to != address(0), "Can't assign to zero address");
        require(_attributeIndexes.length == attributeNames.length, "Need to input all attributes");
        MemberData memory member;
        member.hasToken = true;
        currentHolders[_to] = member;
        for (uint256 index = 0; index < _attributeIndexes.length; index++) {
